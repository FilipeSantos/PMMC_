<?php
    ...
    class BancoArquivo {
        ...
        public function set_bancoArquivo($titulo, $formato, $thumb, $arquivo, $status){
            $valida = new Valida();
        ...
        public function salvar(){
            ...
            if($this->id){
                if($rs = mysql_query("update `arquivo` set titulo = $this->titulo, thumb = $this->thumb, arquivo = $this->arquivo,
                                     ...
                                     limit 1;")){
                    if(file_exists($_SERVER['DOCUMENT_ROOT'] . '/upload/temp/' . str_replace('\'', '', $this->arquivo)) === FALSE){
                        $img = new Upload();
                        $img->set_dir_temp('/upload/temp');
                        $img->set_dir_upload('/upload/banco-de-arquivos');
