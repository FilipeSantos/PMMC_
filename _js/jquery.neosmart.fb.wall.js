	...
	$.fn.fbWall = function(options) {
		...
		return meta.each(function() {
			...
			var initWall = function(data){
				...
				for(var k=0;k<max;k++){
					if(o.showId && o.showId != data[k].from.id ){
						continue;
					}
					...
					// Box -----------------------------------------------------------------------------------------------------------------------------------
					output += '<div class="fb-wall-box' + (!hasItem ? ' fb-wall-box-first' : '') + '">';
				...
				meta.hide().html(output).find('.fb-wall-box:last').addClass('fb-wall-box-last').end().fadeIn(700);
			}
