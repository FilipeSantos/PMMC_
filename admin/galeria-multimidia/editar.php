...
?>
   ...
   <body class="releases">
      <div id="wrapper">
         ...
         <div id="contentTb">
            <h2>Galeria Multimídia - Editar</h2>
            <div class="infoSecao">
               <p>* Campos Obrigatórios</p>
            </div>
            <?php
               $id = isset($_GET['id']) && is_numeric($_GET['id']) ? addslashes($_GET['id']) : FALSE;
               if($id){
                  $results = Galeria::get_galeria($id, FALSE, TRUE, TRUE);
                  foreach($results as $item){
                     require_once($_SERVER['DOCUMENT_ROOT'] . '/admin/_inc/formGaleria.php');
                  }
               }
            ?>
         </div>
