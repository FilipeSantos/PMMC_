	...
	if($cache->cached === FALSE):
		...
		if($programacao !== FALSE){
			...
			foreach($programacao as $item){
				...
				if(strpos($item->divisao, '1') !== FALSE || strpos($item->divisao, '2') !== FALSE){
					$arrDivPrincipal[] = $item;
				} elseif(strpos($item->divisao, '3') !== FALSE){
					$arrDivEsp[] = $item;
				} elseif(strpos($item->divisao, '4') !== FALSE){
					$arrDivExt[] = $item;
				}
			}
		}
...
jQuery(function($) {
	...
	$('#participantes').selectmenu({ style:'dropdown', width: 300, maxHeight: 230});
			...
			<div id="boxSelectCid">
				...
				<select name="participantes" id="participantes" autocomplete="off">
					<?php foreach($listCidades as $listCidade) : ?>
						<option value="<?php echo $listCidade->slug; ?>"<?php echo $cidade->slug === $listCidade->slug ? ' selected="selected"' : ''; ?>><?php echo $listCidade->nome; ?></option>
					<?php endforeach; ?>
					</select>
				...
				<form id="frmParticipantes">
					<div class="r">
						<div class="partQuadro">
							...
							<ul>
								...
								<?php else : ?>
									...
									<?php /* 
									<li class="scroll-pane" style="height: <?php echo ($totalModMedalhasCid <= 4) ? $totalModMedalhasCid * 40 : 160 ; ?>px;">
										<ul>
						...
						<div class="partClassif ">
							...
							<p>
								<a class="veja" href="/resultados/classificacao">Veja a classificação completa</a>
							...
							<?php if(in_array('1', $divisaoCidade) || in_array('2', $divisaoCidade)) : ?>
								<div class="tab" id="box1a">
									<div class="prog-title-cidades prog-title">
										<a href="#<?php echo date('d-m-Y', strtotime('-1 day')); ?>" class="prog-prev<?php echo strtotime('-1 day') < mktime(0,0,0,11,7,11) ? ' hide' : '' ; ?>">&nbsp;</a>
										<span class="data dataFilter" rel="<?php echo date('d-m-Y'); ?>"><?php echo date('d\/m') . ' - ' . Utility::data_extenso_texto(date('N')); ?></span>
										<a href="#<?php echo date('d-m-Y', strtotime('+1 day')); ?>" class="prog-next<?php echo strtotime('+1 day') > mktime(23,59,59,11,19,11) ? ' hide' : '' ; ?>">&nbsp;</a>
									</div>
									
									<div class="scroll-pane">
										<div id="containerDivPrincipal">
										...
										<?php else : foreach($arrDivPrincipal as $i=>$item) : ?>
											<div class="item itemResultCid<?php echo $i % 2 + 1; ?>">
												...
												<p>
													<span><b><?php echo date('H:i', htmlspecialchars($item->data)); ?>h: <a class="btnBold" href="/modalidade/<?php echo htmlspecialchars($item->slug_modalidade); ?>"><?php echo htmlspecialchars($item->modalidade); ?></a>
													<?php echo (!empty($item->descricao) || !empty($item->provas)) ? ' - ' : ''; ?>
														<?php echo isset($item->provas) ? htmlspecialchars($item->provas) : ''; ?>
							...
							<ul>
								<?php foreach($noticias as $noticia) : ?>
									<li>
										...
										<?php if(!empty($noticia->imagemThumb)) : ?>
										...
										<?php endif; ?>
										<div class="boxTxt<?php echo empty($noticia->imagemThumb) ? ' boxTxtNoImg' : ''; ?>">
